...
{
    public class AlmacenUbicacionDat : IAlmacenUbicacionDat
    {
        public async Task<List<AlmacenUbicacionDTO>> Listar()
        {
            try
            {
                ...
                using SqlCommand cmd = new SqlCommand("LG_SP_AlmacenUbicacion_Listar", conn)
                {
        ...
        public async Task<bool> Registrar(AlmacenUbicacionDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_AlmacenUbicacion_Registrar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Actualizar(int id, AlmacenUbicacionDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_AlmacenUbicacion_Modificar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        public async Task<List<AlmacenUbicacionDTO>> BuscarPorAlmacen(int idAlmacen)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_AlmacenUbicacion_ListarPorAlmacen", conn)
                {
                    CommandType = CommandType.StoredProcedure   
                };
                cmd.Parameters.AddWithValue("pIdAlmacen", idAlmacen);
                var reader = await cmd.ExecuteReaderAsync();
                var output = await ReadListar(reader);
        ...
        static async Task<List<AlmacenUbicacionDTO>> ReadListar(DbDataReader reader)
        {
            try
            {
                List<AlmacenUbicacionDTO> collection = new List<AlmacenUbicacionDTO>();
                while (await reader.ReadAsync())
                {
                    AlmacenUbicacionDTO obj = new AlmacenUbicacionDTO();
