...
{
    public class ArticuloCategoriaDat : IArticuloCategoriaDat
    {
        public async Task<List<ArticuloCategoriaDTO>> Listar()
        {
            try
            {
                ...
                using SqlCommand cmd = new SqlCommand("LG_SP_ArticuloCategoria_Listar", conn)
                {
        ...
        public async Task<bool> Registrar(ArticuloCategoriaDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_ArticuloCategoria_Registrar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Actualizar(int id, ArticuloCategoriaDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_ArticuloCategoria_Modificar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        static async Task<List<ArticuloCategoriaDTO>> ReadListar(DbDataReader reader)
        {
            try
            {
                List<ArticuloCategoriaDTO> collection = new List<ArticuloCategoriaDTO>();
                while (await reader.ReadAsync())
                {
                    ArticuloCategoriaDTO obj = new ArticuloCategoriaDTO();
