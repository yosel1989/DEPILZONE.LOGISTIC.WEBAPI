...
{
    public class UnidadMedidaDat : IUnidadMedidaDat
    {
        public async Task<List<UnidadMedidaDTO>> Listar()
        {
            try
            {
                ...
                using SqlCommand cmd = new SqlCommand("LG_SP_UnidadMedida_Listar", conn)
                {
        ...
        public async Task<bool> Registrar(UnidadMedidaDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_UnidadMedida_Registrar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Actualizar(int id, UnidadMedidaDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_UnidadMedida_Modificar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        static async Task<List<UnidadMedidaDTO>> ReadListar(DbDataReader reader)
        {
            try
            {
                List<UnidadMedidaDTO> collection = new List<UnidadMedidaDTO>();
                while (await reader.ReadAsync())
                {
                    ...
                    obj.NombreCorto = DBNull.Value == reader["NombreCorto"] ? null : Convert.ToString(reader["NombreCorto"]);
