...
{
    public class TransaccionClaseDat : ITransaccionClaseDat
    {
        public async Task<List<TransaccionClaseDTO>> Listar()
        {
            try
            {
                ...
                using SqlCommand cmd = new SqlCommand("LG_SP_TransaccionClase_Listar", conn)
                {
        ...
        public async Task<bool> Registrar(TransaccionClaseDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_TransaccionClase_Registrar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Actualizar(int id, TransaccionClaseDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("LG_SP_TransaccionClase_Modificar", conn)
                {
                    CommandType = CommandType.StoredProcedure
                };
        ...
        static async Task<List<TransaccionClaseDTO>> ReadListar(DbDataReader reader)
        {
            try
            {
                List<TransaccionClaseDTO> collection = new List<TransaccionClaseDTO>();
                while (await reader.ReadAsync())
                {
                    TransaccionClaseDTO obj = new TransaccionClaseDTO();
