...
namespace DepilZone.Api.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<RespuestaDTO<ChatEnt>> Post(ChatEnt model)
        {
            RespuestaDTO<ChatEnt> RespuestaDTO = await _chat.Insertar(model);
            IdentificacionUsuarioChatDTO usuarioDestino = Program.usuarios.Values.Where(x => x.IdUsuario == RespuestaDTO.Response.IdParaUsuario).FirstOrDefault();
            if (usuarioDestino != null)
            {
                MensajeSignalR mensajeSignalR = new MensajeSignalR()
                {
                    Tipo = TipoAlerta.NuevoMensaje,
                    DatosJSON = JsonSerializer.Serialize(RespuestaDTO.Response),
                    Exito = true,
        ...
        [HttpPut]
        public async Task<RespuestaDTO<ChatEnt>> Put(ChatEnt model)
        {
